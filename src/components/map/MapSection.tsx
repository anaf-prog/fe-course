import { ExternalLink, Maximize2, Navigation } from "lucide-react"
import { useState } from "react"

const MapSection = () => {
  const [isExpanded, setIsExpanded] = useState(false)

  // Koordinat Indonesian Drafting School dari iframe Anda
  const latitude = -6.260654361292918
  const longitude = 107.11650327453172
  const placeName = "Indonesian+Drafting+School"
  
  // URL embed dari iframe yang Anda berikan
  ...
  return (
    <section className="py-16 px-4 sm:px-6 lg:px-8 bg-[#030712]">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-10">
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
            Lokasi Indonesian <span className="text-[#FF6C37]">Drafting School</span>
          </h2>
        ...
        <div className="relative rounded-2xl overflow-hidden border border-gray-800 bg-gradient-to-br from-gray-900 to-black p-4 md:p-6">
          {/* Container Peta */}
          <div className={`relative ${isExpanded ? 'fixed inset-0 z-50' : 'rounded-xl overflow-hidden'}`}>
            {isExpanded ? (
              <div className="fixed inset-0 bg-black/95 z-50 flex flex-col">
                <div className="flex justify-between items-center p-4 md:p-6 bg-[#1a1a2e] border-b border-[#FF6C37]/30">
                  <div>
                    <h3 className="text-white text-xl md:text-2xl font-bold">Indonesian Drafting School</h3>
                    <p className="text-[#FF6C37] text-sm md:text-base">Peta Lokasi Kami</p>
                  </div>
                  <button
                    onClick={() => setIsExpanded(false)}
                    className="text-white hover:text-[#FF6C37] p-2 md:p-3 rounded-lg hover:bg-gray-800/50 transition-colors border border-gray-700 hover:border-[#FF6C37]/30"
                  >
              ...
              <>
                <iframe
                  src={mapEmbedUrl}
                  className="w-full aspect-video md:aspect-[21/9] rounded-xl"
                  allowFullScreen
                  loading="lazy"
                  referrerPolicy="no-referrer-when-downgrade"
                ...
                {/* Overlay untuk efek interaktif dengan gradien oranye */}
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/20 via-40% to-transparent pointer-events-none rounded-xl" />
          ...
          {/* Tombol untuk membuka Google Maps - OREN */}
          <button
          ...
          {/* Tombol Petunjuk Arah - OREN VARIASI */}
          <button
          ...
          {/* Info lokasi - SESUAI TEME OREN */}
          <div className="absolute top-4 left-4 md:top-6 md:left-6 bg-gradient-to-br from-gray-900/90 to-gray-950/90 backdrop-blur-sm text-white p-3 md:p-4 rounded-xl max-w-[280px] md:max-w-xs border border-[#FF6C37]/20 shadow-2xl">
            <h3 className="font-bold text-base md:text-lg mb-1 md:mb-2 flex items-center gap-2">
              <svg className="w-4 h-4 md:w-5 md:h-5 text-[#FF6C37] flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
